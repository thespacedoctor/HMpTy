...
PySwigObject_format(const char* fmt, PySwigObject *v)
{
    ...
    if (args) {
        if (PyTuple_SetItem(args, 0, PySwigObject_long(v)) == 0) {
            PyObject *ofmt = PyBytes_FromString(fmt);
            if (ofmt) {
...
#endif
{
    ...
    PyObject *repr = PyBytes_FromFormat("<Swig Object of type '%s' at 0x%s>", name, PyBytes_AsString(hex));
    Py_DECREF(hex);
    if (v->next) {
#ifdef METH_NOARGS
...
PySwigPacked_repr(PySwigPacked *v)
{
    char result[SWIG_BUFFER_SIZE];
    if (SWIG_PackDataName(result, v->pack, v->size, 0, sizeof(result))) {
        return PyBytes_FromFormat("<Swig Packed at %s%s>", result, v->ty->name);
    }
    else {
        return PyBytes_FromFormat("<Swig Packed %s>", v->ty->name);
    }
}
...
PySwigPacked_str(PySwigPacked *v)
{
    char result[SWIG_BUFFER_SIZE];
    if (SWIG_PackDataName(result, v->pack, v->size, 0, sizeof(result))) {
        return PyBytes_FromFormat("%s%s", result, v->ty->name);
    }
    else {
        return PyBytes_FromString(v->ty->name);
    }
}
...
_SWIG_This(void)
{
    return PyBytes_FromString("this");
}
...
SWIG_Python_TypeQuery(const char *type)
{
    ...
    PyObject *key = PyBytes_FromString(type);
